function initializeWebsite() {
    ...
    if (cursor) {
        const interactiveElements = document.querySelectorAll('a, button, .magnetic-link, .project-entry');
    ...
    if (typewriterElement) {
        ...
        function type() {
            ...
            if (isDeleting) { typewriterElement.textContent = currentWord.substring(0, letterIndex - 1); letterIndex--; } 
            else { typewriterElement.textContent = currentWord.substring(0, letterIndex + 1); letterIndex++; }
            let typeSpeed = isDeleting ? 75 : 150;
            if (!isDeleting && letterIndex === currentWord.length) { typeSpeed = 2000; isDeleting = true; } 
            else if (isDeleting && letterIndex === 0) { isDeleting = false; wordIndex = (wordIndex + 1) % words.length; typeSpeed = 500; }
    ...
    if (faqItems.length > 0) {
        faqItems.forEach(item => {
            ...
            question.addEventListener('click', () => {
                ...
                if (!isOpen) { question.classList.add('active'); answer.style.maxHeight = answer.scrollHeight + 'px'; }
            });
        });
    }
}
